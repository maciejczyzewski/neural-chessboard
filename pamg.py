...
NC_PAMG_MODEL = tflearn.DNN(deps.pamg.network())
...
def PAMG(img, lines, size=10):
	print(utils.call("PAMG(img, lines)"))

	__points, points = pamg_intersections(lines), []
	debug.image(img).points(__points, size=3).save("pamg_in_queue")
	...
	for pt in __points:
		...
		points += [pt]
	points = pamg_cluster(points)

	debug.image(img).points(points, size=5, \
		color=debug.color()).save("pamg_good_points")
